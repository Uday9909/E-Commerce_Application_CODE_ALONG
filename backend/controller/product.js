...
router.post('/cart',async(req,res)=>{
    try{
        ...
        console.error(error);
        res.status(500).json({error: "Server error. Could not add product to cart"});
    }
});
...
router.get('/cartproducts',async(req,res)=>{
    try{
        ...
        const user=await User.cart.findOne({email}).populate({
            path:'cart.productId',
            model:'Product'
        });
        if(!user){
        ...
        console.error('Server error',err);
        res.status(500).json({error: "Server error. Could not retrieve cart products"});
    }
})
